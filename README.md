...
- `/api/jobs/:id/generate-script` calls Groq (`llama-3.3-70b-versatile`) to turn the
  data into a scene-by-scene video script (headline, on-screen text, voiceover lines,
  durations) — no data is invented, every number/name must match what was entered
...
```
npm install
cp .dev.vars.example .dev.vars   # fill in GROQ_API_KEY (from console.groq.com)
...
npm run deploy
```
Then set the `GROQ_API_KEY` secret on the deployed Worker:
```
